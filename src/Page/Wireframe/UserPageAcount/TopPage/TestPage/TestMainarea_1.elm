module Page.Wireframe.UserPageAcount.TopPage.TestPage.TestMainarea_1 exposing (..)
    ...
    Style.styleSheet
        ...
        , style BKImage
            [ Background.imageWith
                ...
                , position = ( ( ( model.width / 13 ) * 8 ), 0.0 ) --[空白:画像=8:5]の為、表示開始位置をずらす
                , repeat = Background.noRepeat
                , size = Background.size
                    { height = ( px ( model.height - 84 ) ) --header分の84pxマイナス
                    , width = ( px ( ( model.width / 13 ) * 5 ) ) --[空白:画像=8:5]の為、幅を変更
                    }
                }
            ]
        , style Button1
